...
fn to_func(ident: String, args: Vec<Expr>, is_const: IsConst) -> Expr {
...
// TODO: Implement length checking for args
fn wrap_with_func(ident: String, mut args: Vec<Expr>) -> Expr {
    use std::mem;
    let arg0 = mem::take(&mut args[0]);

    match ident.as_str() {
        "sin" => Expr::new_sin(arg0),
        "cos" => Expr::new_cos(arg0),
        "ln" => Expr::new_log(f32::consts::E, arg0),
        "log" => if args.len() == 2 {
            let arg1 = mem::take(&mut args[1]);
            Expr::new_log(arg0, arg1)
        } else {
            Expr::new_log(Expr::Num(10.0), arg0)
        },
